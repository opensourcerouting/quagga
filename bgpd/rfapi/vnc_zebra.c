    ...
    int				add)	/* !0 = add, 0 = del */
{
  ...
  if (!zclient_vnc->redist[ZEBRA_ROUTE_VNC])
    return;
...
//  bgp->redist[afi][type] = 1;
  ...
  if (zclient_vnc->redist[type])
    return CMD_WARNING;

  vrf_bitmap_set (zclient_vnc->redist[type], VRF_DEFAULT);

  //zclient_vnc->redist[type] = 1; 
  ...
  zebra_redistribute_send (ZEBRA_REDISTRIBUTE_ADD, zclient_vnc, type, VRF_DEFAULT);
...
vnc_redistribute_unset (struct bgp *bgp, afi_t afi, int type)
{
  ...
  if (bgp->rfapi_cfg->redist[AFI_IP][type] == 0
      && bgp->rfapi_cfg->redist[AFI_IP6][type] == 0 && zclient_vnc->sock >= 0)
    {
      ...
      zebra_redistribute_send (ZEBRA_REDISTRIBUTE_DELETE, zclient_vnc, type,
                               VRF_DEFAULT);
    }
...
vnc_zebra_init (struct thread_master *master)
{
  ...
  zclient_init (zclient_vnc, ZEBRA_ROUTE_VNC);

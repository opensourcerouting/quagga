...
rip_interface_down (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (IS_RIP_DEBUG_ZEBRA)
    zlog_debug ("interface %s index %d flags %#"PRIx64" metric %d mtu %d is down",
...
rip_interface_up (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (IS_RIP_DEBUG_ZEBRA)
    zlog_debug ("interface %s index %d flags %#"PRIx64" metric %d mtu %d is up",
	       ifp->name, ifp->ifindex, ifp->flags, ifp->metric, ifp->mtu);
...
rip_interface_add (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (IS_RIP_DEBUG_ZEBRA)
    zlog_debug ("interface add %s index %d flags %#"PRIx64" metric %d mtu %d",
		ifp->name, ifp->ifindex, ifp->flags, ifp->metric, ifp->mtu);
       ...
       "Clear text authentication\n")
{
  ...
  if (0 == (hash_algo = hash_algo_byname (argv[0]))) /* "text" */
  {

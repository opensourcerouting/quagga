...
lsa_link_broadcast_set (struct stream *s, struct ospf_interface *oi)
{
  ...
  if (dr && (dr->state == NSM_Full ||
    ...
    {
      if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
        zlog_debug ("LSA[Type1]: Interface %s is in state Full and has "
                    "DR. Adding transit interface", oi->ifp->name);
  ...
  /* Describe type 3 link. */
    ...
    {
      if (IS_DEBUG_OSPF (lsa, LSA_GENERATE))
        zlog_debug ("LSA[Type1]: Interface %s is in state Full but has "
                    "no DR. Adding stub interface", oi->ifp->name);
      masklen2ip (oi->address->prefixlen, &mask);
      id.s_addr = oi->address->u.prefix4.s_addr & mask.s_addr;
      return link_info_set (s, id, mask, LSA_LINK_TYPE_STUB, 0,
                            oi->output_cost);
